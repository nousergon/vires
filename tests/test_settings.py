...
def test_settings_update_partial(client):
    r = client.put("/app/api/settings", json={"weight_unit": "kg", "default_rest_seconds": 120})
    assert r.status_code == 200
    body = r.json()
    ...
    # persisted
    assert client.get("/app/api/settings").json()["weight_unit"] == "kg"


def test_settings_validation(client):
    assert client.put("/app/api/settings", json={"weight_unit": "stone"}).status_code == 422
    assert client.put("/app/api/settings", json={"default_rest_seconds": -5}).status_code == 422
    assert client.put("/app/api/settings", json={"default_reps": 0}).status_code == 422


def test_timer_alert_pref_defaults(client):
    body = client.get("/app/api/settings").json()
...
def test_timer_alert_prefs_update(client):
    ...
    # persisted
    assert client.get("/app/api/settings").json()["timer_notification"] is True


def test_preferred_weekdays_defaults_empty(client):
    assert client.get("/app/api/settings").json()["preferred_weekdays"] == []


def test_preferred_weekdays_update_and_persist(client):
    r = client.put("/app/api/settings", json={"preferred_weekdays": ["monday", "thursday"]})
    assert r.status_code == 200
    assert r.json()["preferred_weekdays"] == ["monday", "thursday"]
    # persisted
    assert client.get("/app/api/settings").json()["preferred_weekdays"] == ["monday", "thursday"]
    # untouched by an unrelated update
    r2 = client.put("/app/api/settings", json={"weight_unit": "kg"})
    assert r2.json()["preferred_weekdays"] == ["monday", "thursday"]


def test_preferred_weekdays_rejects_invalid_day(client):
    r = client.put("/app/api/settings", json={"preferred_weekdays": ["someday"]})
