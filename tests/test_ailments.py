...
def test_pending_check_ins_excludes_checked_today(client):
    ...
    pending = client.get("/app/api/ailments/pending-check-ins", params={"date": today}).json()
    assert pending == []
    # New day with no check-in should appear pending
    tomorrow = (date.today() + timedelta(days=1)).isoformat()
    pending2 = client.get("/app/api/ailments/pending-check-ins", params={"date": tomorrow}).json()
    assert len(pending2) == 1
    assert pending2[0]["ailment"]["id"] == created["id"]


def test_add_check_in_updates_severity(client):
    ep = client.post("/app/api/ailments", json={"label": "Disc"}).json()
    r = client.post(
        f"/app/api/ailments/{ep['id']}/check-ins",
        json={"severity": 5, "note": "Walking hurts"},
    )
    assert r.status_code == 201
    assert r.json()["severity"] == 5
    listed = client.get("/app/api/ailments").json()
    assert listed[0]["latest_severity"] == 5


def test_resolve_ailment(client):
    ep = client.post("/app/api/ailments", json={"label": "Knee"}).json()
    updated = client.patch(f"/app/api/ailments/{ep['id']}", json={"status": "resolved"}).json()
    assert updated["status"] == "resolved"
    assert updated["resolved_at"] == date.today().isoformat()
    assert client.get("/app/api/ailments", params={"status": "open"}).json() == []


def test_check_in_on_resolved_fails(client):
    ep = client.post("/app/api/ailments", json={"label": "Knee"}).json()
    client.patch(f"/app/api/ailments/{ep['id']}", json={"status": "resolved"})
    r = client.post(f"/app/api/ailments/{ep['id']}/check-ins", json={"severity": 1})
