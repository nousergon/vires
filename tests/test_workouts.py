...
def test_empty_workout_log_and_finish(client):
    ...
    ws = client.post("/app/api/workouts", json={"name": "Quick"}).json()
    ...
    fin = client.post(f"/app/api/workouts/{ws['id']}/finish").json()
...
def test_ad_hoc_exercise_with_target_sets_seeds_ready_to_fill_rows(client):
    ...
    bare = client.post(
        f"/app/api/workouts/{ws['id']}/exercises", json={"exercise_id": ex}
    ).json()
    assert bare["sets"] == []
...
def test_start_from_template_clones_exercises(client):
    ...
    ws = client.post("/app/api/workouts", json={"template_id": tpl["id"]}).json()
...
def test_starting_ad_hoc_workout_with_knee_severity_seven_surfaces_warning(client):
    ...
    r = client.post("/app/api/workouts", json={"template_id": tpl["id"]})
...
def test_starting_empty_workout_with_knee_severity_eight_is_blocked(client):
    ...
    r = client.post("/app/api/workouts", json={})
    assert r.status_code == 409
...
def test_target_weight_used_despite_blank_prior_history(client):
    ...
    w0 = client.post("/app/api/workouts", json={}).json()
    se0 = client.post(f"/app/api/workouts/{w0['id']}/exercises", json={"exercise_id": e1}).json()
    client.post(f"/app/api/workouts/{w0['id']}/exercises/{se0['id']}/sets", json={"reps": 5})
    client.post(f"/app/api/workouts/{w0['id']}/finish")
...
def test_dumbbell_template_target_weight_seeds_per_hand(client):
    ...
    ws2 = client.post("/app/api/workouts", json={"template_id": tpl2["id"]}).json()
...
def test_mark_set_done_toggles_completed_at(client):
    ...
    se = client.post(f"/app/api/workouts/{ws['id']}/exercises", json={"exercise_id": ex}).json()
    s = client.post(
        f"/app/api/workouts/{ws['id']}/exercises/{se['id']}/sets", json={"reps": 5, "weight": 100}
    ).json()

    done = client.patch(
        f"/app/api/workouts/{ws['id']}/exercises/{se['id']}/sets/{s['id']}",
    ...
    undone = client.patch(
        f"/app/api/workouts/{ws['id']}/exercises/{se['id']}/sets/{s['id']}", json={"done": False}
    ).json()
...
def test_previous_performance_hint(client):
    ...
    w1 = client.post("/app/api/workouts", json={"name": "Day 1"}).json()
    se1 = client.post(f"/app/api/workouts/{w1['id']}/exercises", json={"exercise_id": ex}).json()
    for _ in range(3):
        client.post(
            f"/app/api/workouts/{w1['id']}/exercises/{se1['id']}/sets",
            json={"reps": 10, "weight": 50},
        )
    client.post(f"/app/api/workouts/{w1['id']}/finish")

    # Second session: adding the same exercise surfaces last time's sets.
    w2 = client.post("/app/api/workouts", json={"name": "Day 2"}).json()
    se2 = client.post(f"/app/api/workouts/{w2['id']}/exercises", json={"exercise_id": ex}).json()
...
def test_previous_performance_hint_carries_duration_for_timed_exercise(client):
    ...
    w1 = client.post("/app/api/workouts", json={"name": "Day 1"}).json()
    se1 = client.post(
        f"/app/api/workouts/{w1['id']}/exercises", json={"exercise_id": plank["id"]}
    ).json()
    client.post(
        f"/app/api/workouts/{w1['id']}/exercises/{se1['id']}/sets",
    ...
    w2 = client.post("/app/api/workouts", json={"name": "Day 2"}).json()
    se2 = client.post(
        f"/app/api/workouts/{w2['id']}/exercises", json={"exercise_id": plank["id"]}
    ).json()
    prev = se2["previous_performance"]
...
def test_history_list_and_volume(client):
    ex = _ex_id(client, "barbell deadlift")
    ws = client.post("/app/api/workouts", json={"name": "Vol"}).json()
    se = client.post(f"/app/api/workouts/{ws['id']}/exercises", json={"exercise_id": ex}).json()
    client.post(
        f"/app/api/workouts/{ws['id']}/exercises/{se['id']}/sets", json={"reps": 5, "weight": 100}
    )
...
def test_update_and_delete_set(client):
    ...
    upd = client.patch(
        f"/app/api/workouts/{ws['id']}/exercises/{se['id']}/sets/{s['id']}", json={"weight": 110}
    ).json()
    assert upd["weight"] == 110 and upd["reps"] == 5

    delr = client.delete(f"/app/api/workouts/{ws['id']}/exercises/{se['id']}/sets/{s['id']}")
    assert delr.status_code == 204
    got = client.get(f"/app/api/workouts/{ws['id']}").json()
...
def test_update_session_exercise_rest_seconds(client):
    ...
    se = client.post(f"/app/api/workouts/{ws['id']}/exercises", json={"exercise_id": ex}).json()
    upd = client.patch(
        f"/app/api/workouts/{ws['id']}/exercises/{se['id']}", json={"rest_seconds": 120}
    ).json()
...
def test_reorder_session_exercises_by_swapping_order_index(client):
    e1, e2 = _ex_id(client, "bench press"), _ex_id(client, "squat")
    ws = client.post("/app/api/workouts", json={}).json()
    se1 = client.post(f"/app/api/workouts/{ws['id']}/exercises", json={"exercise_id": e1}).json()
    se2 = client.post(f"/app/api/workouts/{ws['id']}/exercises", json={"exercise_id": e2}).json()
    assert se1["order_index"] == 0 and se2["order_index"] == 1
    # Swap their order; the session should list them in the new order.
    client.patch(f"/app/api/workouts/{ws['id']}/exercises/{se1['id']}", json={"order_index": 1})
    client.patch(f"/app/api/workouts/{ws['id']}/exercises/{se2['id']}", json={"order_index": 0})
    got = client.get(f"/app/api/workouts/{ws['id']}").json()
...
def test_reorder_session_exercises_batch(client):
    ...
    se1 = client.post(f"/app/api/workouts/{ws['id']}/exercises", json={"exercise_id": e1}).json()
    se2 = client.post(f"/app/api/workouts/{ws['id']}/exercises", json={"exercise_id": e2}).json()
    se3 = client.post(f"/app/api/workouts/{ws['id']}/exercises", json={"exercise_id": e3}).json()

    reordered = client.patch(
        f"/app/api/workouts/{ws['id']}/exercises/reorder",
...
def test_workout_404(client):
    assert client.get("/app/api/workouts/99999999").status_code == 404
...
def test_start_workout_with_tags(client):
    ...
    bare = client.post("/app/api/workouts", json={"name": "Bare"}).json()
    assert bare["tags"] == []


def test_workout_tags_endpoint_ranks_by_frequency_then_alpha(client):
    client.post("/app/api/workouts", json={"name": "A", "tags": ["push", "coffee"]})
    client.post("/app/api/workouts", json={"name": "B", "tags": ["push", "banana"]})
    client.post("/app/api/workouts", json={"name": "C", "tags": ["push"]})
    tags = client.get("/app/api/workouts/tags").json()
...
def test_finish_records_energy_intensity_and_challenge(client):
    ws = client.post("/app/api/workouts", json={"name": "Legs"}).json()
    fin = client.post(
        f"/app/api/workouts/{ws['id']}/finish",
...
def test_finish_ratings_out_of_range_rejected(client):
    ws = client.post("/app/api/workouts", json={"name": "X"}).json()
    assert (
        client.post(
            f"/app/api/workouts/{ws['id']}/finish", json={"energy_level": 11}
        ).status_code
        == 422
    )
    assert (
        client.post(
            f"/app/api/workouts/{ws['id']}/finish", json={"workout_intensity": 0}
        ).status_code
        == 422
    )
    assert (
        client.post(
            f"/app/api/workouts/{ws['id']}/finish", json={"challenge_level": 0}
        ).status_code
        == 422
    )
...
def test_finish_without_body_still_closes_session(client):
    ws = client.post("/app/api/workouts", json={"name": "NoRating"}).json()
    fin = client.post(f"/app/api/workouts/{ws['id']}/finish").json()
    assert fin["ended_at"] is not None
...
def test_ratings_can_be_filled_in_after_finishing(client):
    ws = client.post("/app/api/workouts", json={"name": "Later"}).json()
    client.post(f"/app/api/workouts/{ws['id']}/finish")  # closed, no ratings
    fin = client.post(
        f"/app/api/workouts/{ws['id']}/finish",
...
def test_patch_session_tracking_fields(client):
    ws = client.post("/app/api/workouts", json={"name": "Edit me"}).json()
...
def test_tracking_fields_surface_in_history_list(client):
    ws = client.post(
        "/app/api/workouts", json={"name": "Traced", "tags": ["morning"]}
    ).json()
    client.post(
        f"/app/api/workouts/{ws['id']}/finish",
        json={"energy_level": 6, "workout_intensity": 7, "challenge_level": 9},
    )
    row = next(w for w in client.get("/app/api/workouts").json() if w["id"] == ws["id"])
...
def _session_with_exercise(client):
    ex = _ex_id(client, "barbell deadlift")
    ws = client.post("/app/api/workouts", json={"name": "Offline"}).json()
    se = client.post(
        f"/app/api/workouts/{ws['id']}/exercises", json={"exercise_id": ex}
    ).json()
    return ws, se
...
def test_replayed_set_is_deduped_on_client_uuid(client):
    ...
    r1 = client.post(
        f"/app/api/workouts/{ws['id']}/exercises/{se['id']}/sets", json=body
    )
    r2 = client.post(
        f"/app/api/workouts/{ws['id']}/exercises/{se['id']}/sets", json=body
    )
    ...
    fin = client.get(f"/app/api/workouts/{ws['id']}").json()
...
def test_same_uuid_under_different_exercises_is_not_deduped(client):
    ...
    se2 = client.post(
        f"/app/api/workouts/{ws['id']}/exercises", json={"exercise_id": ex2}
    ).json()
    uuid = "cccccccc-cccc-4ccc-8ccc-cccccccccccc"
    r1 = client.post(
        f"/app/api/workouts/{ws['id']}/exercises/{se1['id']}/sets",
        json={"reps": 5, "client_uuid": uuid},
    )
    r2 = client.post(
        f"/app/api/workouts/{ws['id']}/exercises/{se2['id']}/sets",
        json={"reps": 5, "client_uuid": uuid},
    )
    assert r1.status_code == 201 and r2.status_code == 201
