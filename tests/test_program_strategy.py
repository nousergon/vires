...
def _template(client) -> int:
    ex = client.get("/app/api/exercises/search", params={"q": "bench press"}).json()[0]
    return client.post(
        "/app/api/templates",
        json={
            "name": "Upper",
...
def test_saved_program_links_objective_and_exposes_summary(client):
    ...
    prog = client.post("/app/api/coach/programs", json={"spec": _spec(tid)}).json()
    ...
    summ = client.get("/app/api/plan/programs").json()[0]
...
def test_deleting_objective_nulls_program_link_but_keeps_plan(client):
    ...
    assert client.delete(f"/app/api/objectives/{oid}").status_code == 204

    # the plan survives; only the link is nulled (ON DELETE SET NULL)
    summ = client.get("/app/api/plan/programs").json()
