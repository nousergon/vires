...
def _changes(client, prog) -> list[dict]:
    return client.get(f"/api/coach/programs/{prog['id']}/changes").json()
...
def test_applying_a_revision_writes_an_audit_event(client):
    ...
    r = client.put(f"/api/coach/programs/{prog['id']}", json={"spec": _spec(tpl_id)})
...
def test_changes_404_for_unknown_program(client):
    assert client.get("/api/coach/programs/999999/changes").status_code == 404
