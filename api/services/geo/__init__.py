"""Geospatial helpers for deriving activity route stats (walk/run/hike).
...
same ``distance_m`` + ``elevation_gain_m`` the activity log wants. Every flexible
input mode reduces to "geometry → stats", so they all funnel through here and
then through the one editable-fields → ``POST /api/workouts/activity`` seam.
...
outage degrades to manual entry, it never blocks logging an activity.
"""
